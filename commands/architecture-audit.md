...
if [ -f "scripts/check-plugin-health.py" ]; then
  python3 scripts/check-plugin-health.py
fi

if [ -f "scripts/check-platform-parity.py" ]; then
  python3 scripts/check-platform-parity.py
fi

if [ -f "scripts/doctor-docs.py" ]; then
  python3 scripts/doctor-docs.py
fi
```
...
- If moderate: "Consider addressing during nearby work"
