---
description: Walk through user experience before scoping (experiential thinking)
...
**Otherwise:**
- Look for planning context automatically (same pattern as scope)
...
2. Check for brainstorm documents:
   ```bash
   ls docs/planning/brainstorm-*.md 2>/dev/null
   ```

3. If found, read and use as the basis for the walkthrough.

4. If neither exists, ask the user for requirements directly.

**Detect software type** from the brainstorm or codebase. Auto-detect or **use AskUserQuestion** to confirm:
...
**Use AskUserQuestion** if the persona is unclear. Do not assume.
...
1. **Present the moment** — Describe the scenario and ask 2-4 concrete questions using AskUserQuestion. Act as a sidekick (Wizard of Oz stance), not a director. Frame questions around what happens, not what the user "should" do.

2. **User responds** with their vision for how the product behaves.

3. **Create an interaction sketch** in the detected software type's format. The sketch should be rough and concrete — not polished, but specific enough to evaluate. Include it directly in the walkthrough.

4. **Apply the 4 cognitive walkthrough questions** as validation:
   - **Q1 — Right goal:** Would the user try to do this? (Do they understand what to do?)
   - **Q2 — Visibility:** Would the user notice the correct action is available? (Is the UI element visible/findable?)
   - **Q3 — Label clarity:** Would the user associate the action with their desired result? (Are labels clear?)
   - **Q4 — Feedback:** Would the user see progress toward their goal? (Does feedback confirm success?)

   If any question fails, flag it and suggest a fix.

5. **Record decisions with provenance:**
   - `user` — explicitly stated by the user
   - `suggested` — AI proposed, user accepted
   - `inferred` — derived from context

**Rubber-stamp guard:** If 3+ consecutive decisions have provenance `suggested`, pause and use AskUserQuestion to prompt the user to drive. Say: "I've been making several suggestions in a row — what's YOUR vision for this part?"
...
**Question:** "Walkthrough complete. How would you like to proceed?"
**Options:**
...
- User explicitly asks to skip to scoping
...
This command will:
1. Load brainstorm context and detect software type
2. Establish the persona lens
3. Walk through 5 experience beats interactively
4. Review against anti-pattern taxonomy
5. Validate scenario checklist
6. Write walkthrough document
7. Offer to continue to scoping

---

## Technique References

This command synthesizes five established UX/design techniques:

- **Cognitive Walkthrough** (Lewis et al. 1990, NN/g) — 4 validation questions at each beat
- **Wizard of Oz Simulation** (game-spice) — AI as sidekick, decision provenance tracking
- **Scenario-Based Design** (Rosson & Carroll) — Persona lens, concrete scenario framing
- **Lo-Fi Prototyping** (IxDF, Figma) — Rough interaction sketches adapted to software type
- **Pretotyping** (Alberto Savoia) — Validate "the right it" before building it right
