...
Usage:
    ./scripts/check-platform-parity.py              # Human-readable report
    ./scripts/check-platform-parity.py --json       # JSON output for CI
    ./scripts/check-platform-parity.py --strict     # Exit 1 on warnings
    ./scripts/check-platform-parity.py --check commands  # Commands only
    ./scripts/check-platform-parity.py --check agents    # Agents only
...
def discover_opencode_assets(repo_root: Path) -> PlatformAssets:
    ...
    # Commands: line-cook-opencode/commands/line-*.md
    commands_dir = repo_root / "line-cook-opencode" / "commands"
    if commands_dir.exists():
    ...
    # Agents: line-cook-opencode/agents/*.md (if they exist)
    agents_dir = repo_root / "line-cook-opencode" / "agents"
    if agents_dir.exists():
        for agent_file in agents_dir.glob("*.md"):
            assets.agents.add(agent_file.stem)
...
def discover_kiro_assets(repo_root: Path) -> PlatformAssets:
    ...
    # Agents: line-cook-kiro/agents/*.json
    agents_dir = repo_root / "line-cook-kiro" / "agents"
    if agents_dir.exists():
...
def check_agent_parity(
    ...
    if not opencode.agents:
        result.warnings.append(
            ...
            "but line-cook-opencode hasn't implemented them)"
        )
    else:
        for agent in SHARED_AGENTS:
